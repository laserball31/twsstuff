...
_goggles = ["G_Balaclava_blk","G_Balaclava_combat","G_Balaclava_lowprofile","G_Balaclava_oli","G_Bandanna_aviator","G_Bandanna_beast","G_Bandanna_blk","G_Bandanna_khk","G_Bandanna_oli","G_Bandanna_shades","G_Bandanna_sport","G_Bandanna_tan","U_I_GhillieSuit","U_O_GhillieSuit"];
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 5}) then {
			_text = switch (true) do {
				case (_x in (units grpPlayer) && playerSide == civilian && (((goggles _x) in _goggles) || ((headgear _x) in _goggles))): {format["<t color='#000000'>Masked Player</t>"];};
