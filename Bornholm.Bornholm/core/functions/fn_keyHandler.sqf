...
switch (_code) do
{
	...
	case 35:
	{
		...
		if(!_shift && _ctrlKey && !isNil "life_curWep_h") then {
	...
	{
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1 && player distance (getMarkerPos "safezone_1") > 400 && player distance (getMarkerPos "safezone_2") > 400 && player distance (getMarkerPos "safezone_3") > 400 && player distance (getMarkerPos "safezone_4") > 400) then
		{
	...
	case 48:
	{
	    ...
	    if(_shift) then
        {
			if(playerSide in [west,independent]) then {
				if(vehicle player != player && !life_siren3_active && ((driver vehicle player) == player) && vehicle player isKindOf "Air") then
				{
					[] spawn
					{
					...
					if((_veh getVariable "siren3")) then
					{
						...
						[[_veh],"life_fnc_fortunateSound",nil,true] spawn life_fnc_MP;
					};
				};
			} else {
