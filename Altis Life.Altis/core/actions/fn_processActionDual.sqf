...
if ((_type == "uranium2") && (life_inv_puranium == 0)) exitWith {hint "You have no solvent to dissolve the uranium.";};

//unprocessed item,processed item, cost if no license,Text to display (I.e Processing (percent) ..."
_itemInfo = switch (_type) do
{
    case "uranium2": {["uranium2","puranium","uranium3",6000,"Legal Uranium Dissolution"];};
    case "uranium2b": {["uranium2","ipuranium","uranium3",6000,"Illegal Uranium Dissolution"];};
    default {[]};
};
...
if(_hasLicense) then
{
    ...
    if(player distance _vendor > 10) exitWith {hint "You have to stay within 10 meters."; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    else
{
    ...
    if(life_cash < _cost) exitWith {hint format["You need $%1 without a license to do this!",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    if(life_cash < _cost) exitWith {hint format["You need $%1 to do this without a license!",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(!([false,_oldItem1,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(!([false,_oldItem2,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(!([true,_newItem,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; [true,_oldItem1,_oldVal] call life_fnc_handleInv; [true,_oldItem2,_oldVal] call life_fnc_handleInv; life_is_processing = false;};
    5 cutText ["","PLAIN"];
    titleText[format["You turned %1 %2 to $%3",_oldVal,_itemName,[_cost] call life_fnc_numberText],"PLAIN"];
