...
if((nearestObject [[9704.23,9792.95,0],"Land_i_Barracks_V2_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	[[[1,2],localize "STR_ISTR_Bolt_AlertFed"],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
	[[[1,2],localize "STR_ISTR_Bolt_AlertFed"],"life_fnc_broadcast",independent,false] spawn life_fnc_MP;
} else {
	[[0,format[localize "STR_ISTR_Bolt_AlertHouse",profileName]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
	[[0,format[localize "STR_ISTR_Bolt_AlertHouse",profileName]],"life_fnc_broadcast",independent,false] spawn life_fnc_MP;
	[[2,format[localize "STR_NOTF_HouseRobbed",(_building getVariable "house_owner") select 1,profileName]],"life_fnc_broadcast",civilian,false] spawn life_fnc_MP;
};
...
switch (typeOf _building) do {
	case "Land_i_Barracks_V2_F": {_cpRate = 0.003;};
...
if((player getVariable["restrained",false]) OR (player getVariable["restrainedCiv",false])) exitWith {life_action_inUse = false;_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
