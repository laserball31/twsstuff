...
if(count _ret > 0) then { life_bail_amount = (_ret select 3); } else { life_bail_amount = 12000; _time = time + (35 * 60); };
_esc = false;
_bail = false;
...
while {true} do
{
	...
	{
		if (((list (efences select _i)) find player > -1) && (!life_istazed) && (!(TJB getVariable ["EiP",false])) ) then {
