...
//		_toolate = 7; //minutes 
//		if (serverTime < (14360 - (_toolate*60))) then {
//			life_is_arrested = true;
//			[0,"!Combat log and you will go to Jail and be unable to Bail out for 15 minutes instead of 5!  !WAIT TO CLICK ABORT!"] spawn life_fnc_broadcast;
//		};
...
_canUseControls = {
	...
	if((player getVariable["restrained",FALSE]) OR (player getVariable["restrainedCiv",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (life_is_arrested) OR (life_istazed)) then {false} else {true};
};
